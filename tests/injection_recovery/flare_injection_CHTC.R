...
source("./CeleriteQFD/R/simuFlares.R") # Simulation function
...
res_path <- "./CeleriteQFD/Results/Sim_inj_recovery/" #base_dir
# Flare simulation parameters
xm <- 10
alpha <- 1
offset <- 30
upper <- 150
...
modelQFD <- stan_model(file = './CeleriteQFD/Stan/Morphology/QFD/CeleriteQFDexN.stan', 
            ...
            includes = paste0('\n#include "', 
                             file.path('./CeleriteQFD/celerite2/celerite2.hpp'), '"\n'))
...
set.seed(31381302) # set.seed(as.numeric(args[1])+as.numeric(args[2]))
for(i in 1:n_rep){
  # Simulate and inject flares
