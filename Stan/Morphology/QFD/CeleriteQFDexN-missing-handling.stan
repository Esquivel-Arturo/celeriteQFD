...
data {
    ...
    int<lower = 0, upper = 1> observed[N]; // a flag for missing observations (1=observed, 0=missing)
...
model{
    ...
    // quiet state
    gamma[1,1] = observed[2] * normal_lpdf(yd[2]|0, sqrt(sigma2_noise));
    // firing state, a (positive tranded) Gaussian random walk 
    gamma[1,2] = observed[2] * exp_mod_normal_lpdf(yd[2] | yd[1] , sqrt(sigma2_noise) , rate_firing );
    // decay state, exponential decay
    gamma[1,3] = observed[2] * normal_lpdf(yd[2] | rate_decay * yd[1], sqrt(sigma2_noise)); 
    ...
    for(tt in 2:(N-1)){
        ...
        accu_quiet[1] = gamma[tt-1,1] + log(theta_quiet[1]) + 
                  observed[tt+1] * normal_lpdf(yd[tt+1]|0, sqrt(sigma2_noise));
        ...
        accu_firing[1] = gamma[tt-1,1] + log(theta_quiet[2]) + 
                  observed[tt+1] * exp_mod_normal_lpdf(yd[tt+1] | yd[tt], sqrt(sigma2_noise) , rate_firing );
        ...
        accu_decay[1] = gamma[tt-1, 2] + log(theta_firing[2]) + 
                  observed[tt+1] * normal_lpdf(yd[tt+1] | rate_decay * yd[tt] , sqrt(sigma2_noise));
        //    came from decay state
...
generated quantities {
   ...
   sigma2 = sqrt(S2 * w2 * Q2);
    ...
    {   
        ...
        // quiet
        best_logp[1,1] = observed[2] * normal_lpdf(yd[2]|0, sqrt(sigma2_noise));
        // firing
        best_logp[1,2] = observed[2] * exp_mod_normal_lpdf(yd[2] | yd[1] , sqrt(sigma2_noise) , rate_firing );
        // decay
        best_logp[1,3] = observed[2] * normal_lpdf(yd[2] | rate_decay * yd[1], sqrt(sigma2_noise)); 
